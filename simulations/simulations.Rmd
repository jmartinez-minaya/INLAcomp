...
names(truth) <- c("x", "y", paste0("field", 1:(D-1)))
...
plot_truth <- lapply(1:(D-1), function(dd){
  csc1 <- colsc(truth[[dd]])
  ggplot() +
    gg(truth, mapping = aes_string("x", "y", fill = paste0("field", dd))) +
    coord_equal() +
...
lin.pred %>%
  apply(., 1, function(z)
    MASS::mvrnorm( n  = 1,
                   ...
                   Sigma = Sigma)) %>%
  t(.)-> alryy
colnames(alryy) <- paste0("alr.yy", (1:(D-1)))
```   
...
```{r}
yy.simplex <- compositions::alrInv(alryy)
yy.simplex <- as.numeric(t(yy.simplex)) %>% matrix(., ncol = D, byrow = TRUE)
colnames(yy.simplex) <- paste0("yy", 1:D)
data <- cbind(data, alryy, yy.simplex)
data %>% head(.)
```
...
bound <- fortify(bnd)
data_plot <- data %>%
  dplyr::select(all_of(c("x", "y", paste0("yy", 1:D)))) %>%
  tidyr::pivot_longer(all_of(paste0("yy", 1:D)))
...
```{r}
 data_plot_alr <- data %>%
    dplyr::select(all_of(c("x", "y", paste0("alr.yy", 1:(D-1))))) %>%
    tidyr::pivot_longer(all_of(paste0("alr.yy", 1:(D-1))))
...
measures <- lapply(model_list, function(mod1){
  ...
  #DIC
  dic.mod1 <- INLAcomp::dic.mult(inf, y = data[, c(paste0("alr.yy", 1:(k-1)))])
  
  #WAIc
  waic.mod1 <- INLAcomp::waic.mult(inf, y = data[, c(paste0("alr.yy", 1:(k-1)))])
...
gridExtra::grid.arrange(arrangeGrob(p1.alr1.Real + theme(legend.position = "none"),
                                    p1.alr1.median + theme(legend.position = "none"),
                                    p1.alr1.025 + theme(legend.position = "none"),
                                    p1.alr1.975 + theme(legend.position = "none"),
                                    nrow = 1),
                        arrangeGrob(mylegend_alr1,
                                    nrow = 1, widths = c(4)),
                        arrangeGrob(p1.alr2.Real + theme(legend.position = "none"),
                                    p1.alr2.median + theme(legend.position = "none"),
                                    p1.alr2.025 + theme(legend.position = "none"),
                                    p1.alr2.975 + theme(legend.position = "none"),
                                    nrow = 1),
                        arrangeGrob(mylegend_alr2, nrow = 1, widths = c(4)),
                        nrow = 4, ncol = 1, heights = c(6, 1, 6, 1))
dev.off()
```
...
lapply(1:(D-1), function(xx){
  data.frame(model$marginals.random$id.xx[[xx]],
             alr = paste0("alr.y", xx),
             var = "beta1")
}) %>% do.call(rbind.data.frame, .) -> data_fixed2
#data_fixed <- rbind(data_fixed, data_fixed2)
data_fixed <- data_fixed2
data_fixed$alrvar = paste0(data_fixed$alr, "-", data_fixed$var)


fixed.real <- data.frame(real = c(as.numeric((betas))),
                         alrvar = c(#paste0("alr.y", 1:(D-1), "-beta0"),
                                    paste0("alr.y", 1:(D-1), "-beta1")))
...
p.hyper <- ggplot(hyper.df) +
  geom_line(aes(x = x, y = y), size = 0.9) +
